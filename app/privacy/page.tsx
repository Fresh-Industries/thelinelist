...
export const metadata: Metadata = pageMetadata({
  title: "Privacy",
  description: "How The Line List handles site, newsletter, introduction, and listing-submission data.",
  path: "/privacy",
});
...
export default function PrivacyPage() {
  return (
    <><SiteHeader /><main id="main"><article className="prose wrap legal-page">
      ...
      <h2>Information you provide</h2><p>Newsletter signup collects an email address. Introduction and listing forms collect the contact, product, production, and manufacturer details you submit. We use those details to respond to your request, review a listing, prevent duplicate submissions, and protect forms from abuse.</p>
      <h2>Where form data goes</h2><p>Depending on deployment configuration, introduction and listing submissions may be stored in Vercel Blob, sent to our operations inbox for review, or held temporarily in server memory. Newsletter emails are sent to the configured newsletter provider, currently Beehiiv or Kit when enabled.</p>
      <h2>Analytics</h2><p>The site can use Plausible or PostHog when configured. We record interaction events such as product selection, filter use, profile views, introduction requests, newsletter signups, and guide-to-directory clicks. PostHog is configured to create person profiles only for identified users.</p>
