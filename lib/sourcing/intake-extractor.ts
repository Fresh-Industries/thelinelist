...
export function extractExplicitFounderFacts(idea: string, source = STARTING_IDEA_SOURCE): AgentFieldUpdate[] {
  ...
  const rejectedBrand = lastMatch(idea, /(?:actually\s+no[^.?!]*brand\s+is\s+undecided[^.?!]*|brand\s+is\s+undecided[^.?!]*|please\s+do\s+not\s+use\s+that\s+name)/gi);
  ...
  if (packageChoice) {
    ...
    if (sizedPackage) {
      ...
      put("product_format", [size, singleServe ? "single-serve" : null, customerContainer].filter(Boolean).join(" "), [sizedPackage, packageChoice], "The founder explicitly described the customer-facing retail format.");
    }
  }
  ...
  if (geography) put("preferred_geography", normalizeGeography(geography[0]), geography, "The founder explicitly named a state or region preference.");
  ...
  const formula = lastMatch(idea, /\b(?:finished|complete|tested)\s+(?:home|kitchen)?\s*recipe\b|\b(?:bench\s+formula\s+exists|draft\s+kitchen\s+recipe[^.?!]*not\s+tested[^.?!]*scale[-\s]ready|(?:home|kitchen)\s+recipe\s+(?:that\s+is\s+)?not\s+(?:scaled|commercialized)|recipe\s+(?:is\s+)?(?:at\s+)?(?:the\s+)?idea\s+stage)\b/gi);
  ...
  const assistance = lastMatch(idea, /\b(?:need|needs|needed|require|requires|required|yes,?\s*i\s+need)\s+(?:help\s+(?:creating|finishing|finalizing|scaling)(?:\s+the)?\s+(?:recipe|formula)|formulation\s+assistance)\b/gi);
  ...
  const allergenMatches = [...allMatches(idea, /\bpeanut[-\s]free\s+(?:is\s+)?(?:a\s+must|required)\b/gi), ...allMatches(idea, /\bno\s+dairy\s+is\s+allowed\b|\bdairy[-\s]free\s+(?:is\s+)?(?:a\s+must|required)\b/gi)].sort(bySourcePosition);
  if (allergenMatches.length) {
    const requirements = [allergenMatches.some((match) => /dairy/i.test(match[0])) ? "Dairy-free" : null, allergenMatches.some((match) => /peanut/i.test(match[0])) ? "peanut-free" : null].filter((value): value is string => Boolean(value));
...
function normalizeFormulaStatus(value: string): string { if (/bench\s+formula/i.test(value)) return "Bench formula"; if (/draft\s+kitchen/i.test(value)) return "Draft kitchen recipe; not tested or scale-ready"; if (/finished|complete|tested/i.test(value)) return "Finished kitchen recipe"; if (/not\s+(?:scaled|commercialized)/i.test(value)) return "Kitchen recipe not scaled for production"; return "Recipe at idea stage"; }
