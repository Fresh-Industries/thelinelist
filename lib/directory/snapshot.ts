...
export function comparableMoq(value: string | null): { amount: number; unit: string } | null {
  ...
  const beforeMatch = value.slice(Math.max(0, matchIndex - 32), matchIndex);
  const afterMatch = value.slice(matchIndex + match[0].length, matchIndex + match[0].length + 24);
  if (
    /(?:[~≈]|\b(?:about|roughly|approximately))\s*$/i.test(beforeMatch)
    || /[\d,.]\s*(?:-|–|—|to)\s*$/i.test(beforeMatch)
    || /^\s*(?:-|–|—|to)\s*[\d,.]/i.test(afterMatch)
    || /^\s*(?:per|\/)\s*(?:day|year|hour|week|month)\b/i.test(afterMatch)
  ) return null;
  const amount = Number(match[1].replace(/,/g, "")) * (match[2] ? 1_000 : 1);
  const unit = MOQ_UNITS[match[3].toLowerCase()];
...
export function categorySnapshot(plants: Plant[]): CategorySnapshot {
  ...
  const comparableMoqRange = comparableGroup
    ? `${Math.min(...comparableGroup[1]).toLocaleString("en-US")}–${Math.max(...comparableGroup[1]).toLocaleString("en-US")} ${comparableGroup[0]} across ${comparableGroup[1].length} published minimums`
    : null;
